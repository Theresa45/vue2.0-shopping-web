<template>
  <div class="detail">
    ...
    <section class="con">
      <!-- 导航路径区域 -->
      <div class="conPoin">
        <span v-show="category.category1Name">{{
          category.category1Name
        }}</span>
        <span v-show="category.category2Name">{{
          category.category2Name
        }}</span>
        <span v-show="category.category3Name">{{
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                ...
                <!-- <router-link @click="addToCart">加入购物车</router-link> -->
...
export default {
  ...
  computed: {
    ...mapGetters(["category", "skuInfo", "spuSaleAttrList"]),
  },
  // 组件一挂载完毕就发送请求获取数据
  mounted() {
  ...
  methods: {
    // 销售属性值的选择
    chooseSpuSaleAttrValue(spuSaleAttrValue, spuSaleAttrValueList) {
      // 清空选择
      spuSaleAttrValueList.forEach(
        (spuSaleAttrValue) => (spuSaleAttrValue.isChecked = "0")
      );
      // 重新选择
    ...
    async addToCart() {
      // 将参数发送给服务器，让服务器添加记录
      // 因为this.$store.dispatch("addOrUpdateShopCart", {skuId: this.skuInfo.skuId,skuNum: this.skuNum})函数在声明的时候使用了async/await所以该函数返回的一定是一个Promise
      try {
        // 成功则继续向下执行，失败则跳转到catch块执行其中的代码
